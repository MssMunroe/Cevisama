...
async function enviarCorreo() {
  ...
  try {
    ...
    const [rows] = await connection.execute("SELECT correo FROM Visitante UNIT SELECT correo FROM Empresa");
  ...
  for (const email of listaDestinatarios) {
    ...
    const mailOptions = {
      from: `"Cevisama" <correo@gmail.com>`,
      to: email,
      subject: "La feria especializada en el sector cerámico esta a la vuelta de la esquina",
      html: contenidoHTML,
